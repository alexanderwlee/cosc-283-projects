 ...
 * @date April 2022
 *     <p>A data link layer accepts a string of bytes, divides it into frames, adds some metadata,
 *     and sends the frame via its physical layer. Upon receiving a frame, the data link layer
 *     removes the metadata, potentially performs some checks on the data, and delivers the data to
 *     its client network layer.
 */
public abstract class DataLinkLayer {
   ...
   * @throws RuntimeException if the given type is not a valid subclass, or if the given physical
   *     layer doesn't exist (is <code>null</code>).
   */
  public static DataLinkLayer create(String type, PhysicalLayer physicalLayer) {
    ...
    try {
      ...
      throw new RuntimeException("Unknown data link layer subclass " + className);
    }
    ...
    try {
      ...
      throw new RuntimeException(className + " is not a subclass of DataLinkLayer");
    }
  ...
  // =========================================================================
  /**
   * Check the status of this connection. If there is buffered data to send, frame and transmit it;
   * if bits are received, process and deliver them one frame at a time.
   */
  ...
  // =========================================================================
  /**
   * Send a sequence of bytes through the physical layer. Expected to be called by the client.
   * Buffers the data; actual sending is triggered by the event loop.
   *
  ...
  // =========================================================================
  /**
   * Deliver a bit into this layer. Expected to be called by the physical layer. Accumulate bits
   * into a buffer, and with each full byte received, accumulate those bits into a byte buffer. Each
   * byte added to the buffer is examined to determine whether a whole frame has been received, and
   * if so, then processed.
   *
   * @param bit The value to receive, where <code>false</code> indicates a <code>0</code>, and
   *     <code>true</code> indicates a <code>1</code>.
   */
  public void receive() {
   ...
   */
  protected abstract Queue<Byte> createFrame(Queue<Byte> data);
  // =========================================================================

  // =========================================================================
  /**
   * Determine whether the byte buffer contains a complete frame. If so, extract its contents,
   * removing all metadata and (if applicable) checking its correctness, then returning (if
   * possible) the contained data.
   *
   * @return if possible, the extracted data from the frame; <code>null</code> otherwise.
   */
  protected abstract Queue<Byte> processFrame();
  // =========================================================================
   ...
   */
  protected abstract void finishFrameSend(Queue<Byte> frame);
  // =========================================================================
   ...
   */
  protected abstract void finishFrameReceive(Queue<Byte> frame);
  // =========================================================================
