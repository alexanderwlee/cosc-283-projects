 ...
 * @date April 2022
 *     <p>A network layer accepts a string of bytes, divides it into packets that contain headers
 *     addressing each packet to a destination host, and then chooses a data link layer to which to
 *     pass the packet for transmission to a neighboring host.
 *     <p>When it receives a packet, it examines the header to determine whether this host is the
 *     destination. If so, it delivers the packet's data to the client; if not, it forwards the
...
public abstract class NetworkLayer {
  ...
  public static NetworkLayer create(String type, Host host) {
    ...
    try {
      ...
      throw new RuntimeException("Unknown network layer subclass " + className);
    }
    ...
    try {
      ...
      throw new RuntimeException(className + " is not a subclass of NetworkLayer");
    }
  ...
  // =========================================================================
  /**
   * Given a link that connects this host to another, attach its data link layer to this network
   * layer.
   *
   * @param dataLinkLayer The data link layer controlling the link.
   * @param hostname The name of the remote host to which this link is connected.
   * @throws RuntimeException if the hostname or the data link layer is a duplicate (or <code>null
   *     </code>).
   */
  public void attach(DataLinkLayer dataLinkLayer, String remoteHostname) {
    ...
    if (dataLinkLayers.containsKey(remoteAddress)) {
      throw new RuntimeException("Cannot attach duplicate host " + remoteHostname);
    }
  ...
  // =========================================================================
  /**
   * The event loop. If there is buffered data to send, construct a packet and pass it to the data
   * link layer for transmission; if data has been delivered from a data link layer, process it.
   */
  public void go() {
  ...
  // =========================================================================
  /**
   * Send a sequence of bytes through this layer. Expected to be called by the client. Packets are
   * constructed and then sent via whichever data link the is chosen by the routing method.
   *
   ...
   */
  protected abstract byte[] createPacket(int destination, byte[] data);
  // =========================================================================
   ...
   */
  protected abstract DataLinkLayer route(int destination);
  // =========================================================================
   ...
   *     otherwise.
   */
  protected abstract byte[] extractPacket(Queue<Byte> buffer);
  // =========================================================================
   ...
   */
  protected abstract void processPacket(byte[] packet);
  // =========================================================================
  ...
  // =========================================================================
  /**
   * Copy bytes into a longer array from a shorter one, copying the entire contents of the shorter
   * array.
   *
  ...
  // =========================================================================
  /**
   * Copy bytes from a longer array into a shorter one, copying into the entire space of the shorter
   * one.
   *
   ...
   * @throws RuntimeException if the data array is <code>null</code> or not of the correct length to
   *     contain an integer's bytes.
   */
